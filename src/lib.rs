...
#[napi]
pub async fn start(arg: String) -> String {
    format!("Start {}", arg)
}

#[napi]
pub async fn peer_connection_1() -> NapiResult<()> {
    ...
    m.register_default_codecs()
        .expect("Error register default codecs");

    let mut registry = Registry::new();

    registry = register_default_interceptors(registry, &mut m)
        .expect("Error register default interceptors");
    ...
    let peer_connection = Arc::new(
        api.new_peer_connection(config)
            .await
            .expect("Error create peer connection"),
    );
    ...
    let data_channel = (peer_connection.create_data_channel("data", None).await)
        .expect("Error create data channel");
    ...
    peer_connection
        ...
        .await
        .expect("Error set local description");
    ...
    let _resp = Client::new()
        .request(req)
        .await
        .expect("Error create response body");
    ...
    peer_connection
        .close()
        .await
        .expect("Error close peer connection");
    Ok(())
}
